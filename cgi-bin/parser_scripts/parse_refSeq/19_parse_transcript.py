...
file_handle = open('%s/data/refSeq/unzipped/ref_GRCh37.p5_top_level.gff3'%root_directory,'r')
out_file = open('%s/parsed_files/19_parsedReft.txt'%root_directory,'w')
...
for l in file_handle:
	...
	try:
		if l[2]=="mRNA":
			...
			transcript=data[1].strip().split('=')[1]#.split('.')[0];
			...
			data=l[8].strip().split(';')
			parent_id=data[2].strip().split('=')[1];
			if parent_id in id_transcript:
				parent_transcript=id_transcript[parent_id];
...
for t in transcript_positions:
        print transcript_positions[t]["sign"]
